...
export default function NewProjectPage() {
  ...
  const handleCreateProject = async () => {
    ...
    try {
      // Using fallback port 8787 for the API
      ...
      if (response.ok) {
        ...
        // FINAL FIX: Explicitly redirecting to the root dashboard
        // The URL in your screenshot shows the ID is being generated correctly!
        window.location.href = `/?project=${data.id}`;
      } else {
        toast.error("Failed to initialize project", { id: toastId });
      }
    } catch (error) {
      toast.error("Connection error. Ensure your API is running.", { id: toastId });
    } finally {
      setIsLoading(false);
    }
  };
          ...
          {step === 1 ? (
            ...
            >
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500/10 border border-blue-500/20 rounded-full text-blue-400 text-sm mb-6">
                {selectedTemplate && <selectedTemplate.icon size={16} />}
