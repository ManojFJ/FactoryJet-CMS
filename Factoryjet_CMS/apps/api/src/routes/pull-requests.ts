...
function generatePRBody(
  ...
  let body = `## 🤖 FactoryJet AI Changes\n\n`;
  ...
  if (added > 0) body += `- ✅ **${added}** file(s) added\n`;
  if (modified > 0) body += `- ✏️ **${modified}** file(s) modified\n`;
  if (deleted > 0) body += `- 🗑️ **${deleted}** file(s) deleted\n`;
  ...
  if (riskFlags.length > 0) {
    ...
    riskFlags.forEach((flag) => {
      const emoji =
        flag.level === 'high' ? '🔴' : flag.level === 'medium' ? '🟡' : '🟢';
  ...
  body += `\n---\n\n`;
  body += `🚀 Generated by [FactoryJet CMS](https://factoryjet.dev)\n`;
